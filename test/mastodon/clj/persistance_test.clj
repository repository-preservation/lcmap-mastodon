...
(deftest ingest-test
  (with-fake-http [{:url "http://www.iwdshost.com/inventory" :method :post} {:status 200 :body "ok"}]
    (with-redefs [config/config {:data_type "ard" :ard_host "foohost.com"}]
...
(deftest resource-path-test-ard
  (with-redefs [config/config {:data_type "ard" :ard_host "http://ardhost.com"}]
...
(deftest resource-path-test-aux
  (with-redefs [config/config {:data_type "aux" :aux_host "http://auxhost.com"}]
